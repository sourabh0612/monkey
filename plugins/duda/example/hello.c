...
void cb_redis_test(duda_request_t *dr)
{
    response->http_status(dr, 200);
    response->http_header(dr, "Content-Type: text/plain", 24);
    ...
    redis->command(rc, getCallback, (char*)"key", "GET key");
...
int duda_main(struct duda_api_objects *api)
{
    ...
    /* URI: /hello/examples/redis_test */
    method = map->method_new("redis_test", "cb_redis_test", 0);
    map->interface_add_method(method, if_system);
